...
def towns
  %w[Rome London Paris NY Vancouver Sydney Bangkok Tokyo
    Beijing Lima Montevideo Caracas Madrid Berlin].freeze
end
...
def exist?(town, strng)
  strng.split("\n").select { |town_strng| town_strng.include?(town) }.empty?
end

def mean(town, strng)
  return -1 if validate_town?(town)
  return -1 if exist?(town, strng)

  average(numbers(town, strng))
end

puts mean('London', data1)
